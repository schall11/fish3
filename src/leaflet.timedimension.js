...
L.TimeDimension = (L.Layer || L.Class).extend({
    ...
    setCurrentTimeIndex: function (newIndex) {
        ...
        var x = new Date(newTime).getTime();
		...
		// console.log(full);
		// if (monDisp == "Mar 2013" ){
        if (x > 1431216000000-1){
		    console.log("MARCH GO");
		    pics.setIcon(picIcon1_second);
        }
        else {
            pics.setIcon(picIcon1_first);
        }
        if ( x > 1430135373000){
            // console.log(x);
		    // console.log("MAYYYSDYSDYS");
		    pics2.addTo(map);
		    leader2.addTo(map);
        }
        else {
            map.removeLayer(pics2);
            map.removeLayer(leader2);
        }
